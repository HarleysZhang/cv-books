## 前言
> These are open and classic books for computer version engineers, it can help us from entry to practice.

**2022-11-9日更新，仓库文件失效了**，建议关注公众号-**嵌入式视觉**，回复相应关键字下载。
本仓库大部分书籍都是**开源**的，通过在网上收集整理而来，如果部分书籍存在侵权行为，可以联系我删除掉相应书籍。

同时，想要知道书籍的**学习重点和路线**建议先看每个目录下的 `README` 图像文件。最后，如果这个仓库对你有所帮助，可以扫码关注下博主的公众号-**嵌入式视觉**。

![公众号二维码](./%E5%85%AC%E4%BC%97%E5%8F%B7-%E5%B5%8C%E5%85%A5%E5%BC%8F%E8%A7%86%E8%A7%89%E4%BA%8C%E7%BB%B4%E7%A0%81.jpg)

## 书籍目录

```shell
├── C++开发
│   ├── C++ Concurrency in Action.pdf
│   ├── C++ How to Program, 8th Edition.pdf
│   ├── C++ Primer (5th Edition).pdf
│   ├── C++ STL源码剖析（侯捷版本）.pdf
│   ├── C++ 工程实践经验谈.pdf
│   ├── C++_标准模板库(STL).pdf
│   ├── C++程序设计语言  第1-3部分_14040196.pdf
│   ├── C++并发编程实战.pdf
│   ├── C++程序设计：现代方法.pdf
│   ├── CMake Practice.pdf
│   ├── Modern C++ Design--Generic Programming and Design Patterns Applied.pdf
│   ├── Muduo 网络库使用手册.pdf
│   ├── Primer C++(第5版).pdf
│   ├── README_C++书籍概述.png
│   ├── modern-cpp-tutorial-en-us.pdf
│   ├── modern-cpp-tutorial-zh-cn.pdf
│   ├── 高质量C++_C编程指南.pdf
│   └── 《现代C++白皮书》电子版.pdf
├── C开发
│   ├── C语言100例及教程.chm
│   ├── C专家编程+(高清晰).pdf
│   └── C语言讲义v1.2.pdf
├── Linux开发
│   └── 鸟哥的Linux私房菜 基础篇 第四版.pdf
├── Lua
│   ├── Lua程序设计(第二版).pdf
│   └── Programming.in.Lua第四版.pdf
├── OpenCV图像处理
│   ├── Learning-OpenCV.pdf
│   ├── OpenCV-Python-Toturial-中文版.pdf
│   ├── OpenCV入门教程（于仕琪）.pdf
│   ├── Python计算机视觉编程.pdf
│   ├── README_OpenCV书籍概述.png
│   ├── opencv-4.1中文官方文档.pdf
│   ├── 学习OpenCV中文版.pdf
│   └── 数字图像处理  第4版=DIGITAL  IMAGE  PROCESSING,FOURTH  EDITION_p734.pdf
├── Python
│   ├── Flask Web开发：基于Python的Web应用开发实战.第2版.pdf
│   ├── Python Web开发实战.pdf
│   ├── Python_Cookbook_3rd_Edition.pdf
│   ├── Python实践资料总结.pdf
│   ├── Python高级编程（第2版）.pdf
│   ├── Python基础教程（第3版）.pdf
│   ├── Python高手之路（第3版）.pdf
│   ├── READM_Python.png
│   ├── 《Python+Cookbook》第三版中文v3.0.0.pdf
│   ├── 《python3教程》-廖雪峰.pdf
│   ├── 流畅的Python.pdf
│   └── 常用数据挖掘算法总结及Python实现.pdf
├── README.md
├── SLAM算法
│   ├── ICE-BA论文分析及GBA代码解析by崔华坤_4.0.pdf
│   ├── PointNet.pdf
│   ├── README_SLAM书籍概述.png
│   ├── ROVIO论文推导及代码解析_V5_byHK20181018.pdf
│   ├── VisualSLAM_Applications.pdf
│   ├── 视觉SLAM十四讲_从理论到实践.pdf
│   ├── 视觉SLAM十四讲从理论到实践第2版.pdf
│   └── 主流VIO技术综述byHK190116.pptx
├── summary.png
├── 公众号-嵌入式视觉二维码.jpg
├── 数据分析
│   ├── Pandas_Cheat_Sheet.pdf
│   ├── joyfulpandas.pdf
│   └── 利用Python进行数据分析 第二版 A5.pdf
├── 机器学习
│   ├── 机器学习(西瓜书).pdf
│   ├── 图解机器学习.pdf
│   └── 机器学习个人笔记完整版v5.32-A4打印版.pdf
├── 深度学习
│   ├── Deep Learning中文版beta(花书).pdf
│   ├── Deep_Learning_for_Computer_Vision_with_Python.pdf
│   ├── Python深度学习.pdf
│   ├── README_深度学习书籍概述.png
│   ├── Tensorflow 实战Google深度学习框架.pdf
│   ├── 动手学深度学习-Pytorch版.pdf
│   ├── 动手学深度学习.pdf
│   └── 神经网络与深度学习-邱锡鹏.pdf
├── 科普书籍
│   ├── PlantUML_Language_Reference_Guide_zh.pdf
│   └── 数学之美  第2版.pdf
├── 视觉算法
│   ├── 202004_VisualSLAM_Navigation.pdf
│   ├── An Introduction to the Kalman Filter.pdf
│   ├── Stereo Vision Algorithms and Applications.pdf
│   ├── 第13章 目标跟踪.pdf
│   └── 多目标跟踪文献综述.pdf
├── 计算机系统
│   ├── Computer Systems A Programmers Perspective (3rd).pdf
│   ├── README.png
│   ├── xUnit Test Patterns_ Refactoring Test Code.pdf
│   ├── 大话设计模式(带目录).pdf
│   ├── 程序员自我修养.pdf
│   ├── 电子设计从零开始(第2版).pdf
│   └── 深入理解计算机系统（第三版）.pdf
├── 高性能计算
│   ├── Introduction to Parallel Computing 2nd.pdf
│   ├── NVIDIA官方2008年-CUDA 编程指南.pdf
│   ├── Processor Microarchitecture An Implementation Perspective.pdf
│   ├── SSE-指令集.pdf
│   ├── SSE指令集补充.pdf
│   ├── parallel-computer
│   │   ├── 01_whyparallelism.pdf
│   │   ├── 02_basicarch.pdf
│   │   ├── 03_progmodels.pdf
│   │   ├── 04_progbasics.pdf
│   │   ├── 05_progperf1.pdf
│   │   ├── 06_gpuarch.pdf
│   │   ├── 07_progperf2.pdf
│   │   ├── 08_casestudies.pdf
│   │   ├── 09_perfeval.pdf
│   │   ├── 10_cachecoherence1.pdf
│   │   ├── 11_directorycoherence.pdf
│   │   ├── 12_snoopimpl.pdf
│   │   ├── 13_perftools.pdf
│   │   ├── 15_consistency.pdf
│   │   ├── 15_interconnects.pdf
│   │   ├── 16_webscaling.pdf
│   │   ├── 17_synchronization.pdf
│   │   ├── 18_lockfree.pdf
│   │   ├── 19_transactionalmem.pdf
│   │   ├── 20_heterogeneity.pdf
│   │   ├── 21_prefetching.pdf
│   │   ├── 22_dsl.pdf
│   │   ├── 23_graphdsl.pdf
│   │   ├── 24_dnn.pdf
│   │   ├── 25a-msgpassing.pdf
│   │   ├── 25b-runtime.pdf
│   │   ├── coher-handout.pdf
│   │   ├── gpu-handout.pdf
│   │   ├── rec_01.pdf
│   │   ├── rec_02.pdf
│   │   ├── rec_03.pdf
│   │   ├── rec_04.pdf
│   │   ├── rec_05.pdf
│   │   ├── spark98.pdf
│   │   └── sync-handout.pdf
│   ├── wangzheng.pdf
│   ├── 并行计算导论 第2版.pdf
│   ├── 并行编程方法与优化实践.pdf
│   └── 让深度学习更高效运行的两个视角.pdf
├── 卷积神经网络
│   └── 解析卷积神经网络-深度学习实践手册.pdf
├── 深度学习框架
│   ├── Caffe官方教程中译本.pdf
│   ├── PyTorch_tutorial_0.0.4_余霆嵩.pdf
│   ├── PyTorch官方教程中文版.pdf
│   ├── README_深度学习框架书籍概述.png
│   └── caffe的基本结构.pdf
├── 数据结构与算法
│   ├── README.png
│   ├── leetcode-cpp.pdf
│   ├── 剑指offer题解c++版.pdf
│   ├── 算法图解.pdf
│   ├── 西法的刷题秘籍-2021-02-17.pdf
│   └── 数据结构与算法学习路线图.jpg
└── 模型压缩与量化
    └── s7310-8-bit-inference-with-tensorrt.pdf
```
